...
const SPECIES_META: Record<SpeciesKey, { name: string; color: string; size: number }> = {
...
function buildTimeline(target: SimTick, baseline: SimTick): SimTick[] {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    const env: EnvState = {
      temperature:
        baseline.environment.temperature +
        (target.environment.temperature - baseline.environment.temperature) * t,
      nutrients:
        baseline.environment.nutrients +
        (target.environment.nutrients - baseline.environment.nutrients) * t,
      pH: baseline.environment.pH + (target.environment.pH - baseline.environment.pH) * t,
      salinity:
        baseline.environment.salinity +
        (target.environment.salinity - baseline.environment.salinity) * t,
      fishing_pressure:
        baseline.environment.fishing_pressure +
        (target.environment.fishing_pressure - baseline.environment.fishing_pressure) * t,
      pollution_index:
        baseline.environment.pollution_index +
        (target.environment.pollution_index - baseline.environment.pollution_index) * t,
    };
...
// ---------------- Helpers ----------------
const trendHex = (t: Trend) =>
  t === "improving" ? "#00ff9d" : t === "stable" ? "#ffb347" : "#ff4757";
...
function EnvReadout({
  ...
  return (
    <div className="rounded-sm bg-white/5 p-2.5">
      <div
        className="text-[10px] tracking-widest text-white/30 font-bold"
...
function SpeciesLegend({ compact = false }: { compact?: boolean }) {
  return (
    <div
      className={`flex flex-wrap gap-x-4 gap-y-2 ${compact ? "text-[10px]" : "text-xs"}`}
      style={{ fontFamily: "var(--font-mono)" }}
    >
      {SPECIES_ORDER.map((sp) => {
        ...
        <div className="w-full max-w-md px-6 flex flex-col gap-3">
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && policy.trim()) onSimulate(policy.trim());
              }}
  ...
  return (
    <div className="fixed inset-0 w-full h-full overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-10 text-center px-6">
        ...
        <div
          className="text-sm md:text-base text-white/40 uppercase tracking-[0.3em] mb-4"
          style={{ fontFamily: "var(--font-mono)" }}
        >
          California Current
        </div>
  ...
  return (
    <div className="rounded-lg bg-[#0a1628]/95 backdrop-blur border border-[#0f2040] shadow-2xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#0f2040] bg-gradient-to-r from-[#0a1628] to-[#0f2040]">
        <div
          className="text-[10px] tracking-[0.2em] font-bold text-[#00cfff]"
          style={{ fontFamily: "var(--font-mono)" }}
        >
          POLICY APPLIED
        </div>
        ...
        {rows.map((r) => {
          ...
          const color =
            dir === "flat"
              ? "#6b8fa8"
              : (dir === "down" && goodDown) ||
                  (dir === "up" && !goodDown && r.key !== "temperature")
                ? "#00ff9d"
                : r.key === "temperature"
                  ? "#00cfff"
                  : "#ff4757";
          ...
          return (
            <div
              ...
              style={{ fontFamily: "var(--font-mono)" }}
            >
              <span className="text-[11px] text-[#e8f4f8] font-medium">
                {PARAM_LABELS[r.key as string]}
              </span>
              <div className="flex items-center gap-2 text-[11px] tabular-nums">
                ...
                <span style={{ color }} className="text-[10px] w-3 text-center">
                  {arrow}
                </span>
              </div>
            </div>
...
function Simulation({
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") {
        setYearIdx((i) => Math.min(timeline.length - 1, i + 1));
        setAutoplay(false);
  ...
  const historyData = timeline.map((t, i) => {
    ...
    SPECIES_ORDER.forEach((s) => {
      row[s] = i <= yearIdx ? t.agents[s].population : null;
    });
    return row;
  });
    ...
    <div key="sim" className="min-h-screen w-full animate-fade-in-slow bg-black text-white">
      <div className="flex items-center justify-between px-6 py-3 border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <span
            className="text-sm text-white/50 font-light italic"
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-xs text-white/40" style={{ fontFamily: "var(--font-mono)" }}>
            YEAR <span className="text-white font-bold">{tick.ticks}</span> /{" "}
      ...
      <div className="flex flex-col lg:flex-row" style={{ height: "calc(100vh - 49px)" }}>
        ...
        <div className="lg:w-[60%] w-full h-[50vh] lg:h-full relative border-r border-white/5">
          <CoastMap tick={tick} onSpeciesClick={(s) => setSelected(s)} showBarrenFlash={barren} />
          {showDiff && (
        ...
        <div className="lg:w-[40%] w-full overflow-y-auto divide-y divide-white/5">
          ...
          <div className="px-5 py-4">
            <div
              className="text-[10px] tracking-[0.2em] font-bold text-white/30 mb-3"
              style={{ fontFamily: "var(--font-mono)" }}
            >
              YEAR {tick.ticks}
            </div>
            ...
            {(barren || collapse) && (
              ...
              >
                <div
                  className="text-[10px] tracking-widest font-bold mb-1"
                  style={{ color: "#ff4757", fontFamily: "var(--font-mono)" }}
                >
                  THRESHOLD EVENT
                </div>
          ...
          <div className="px-5 py-4">
            <div
              className="text-[10px] tracking-[0.2em] font-bold text-white/30 mb-2"
              style={{ fontFamily: "var(--font-mono)" }}
            >
              LEGEND
            </div>
              ...
              {SPECIES_ORDER.map((sp) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <div
                      className="text-xs font-bold tracking-wide text-white/80 mt-2 uppercase"
                      style={{ fontFamily: "var(--font-mono)" }}
                    >
                      {meta.name}
                    </div>
                    ...
                    <div
                      ...
                      style={{ fontFamily: "var(--font-display)", color: trendHex(a.health_trend) }}
                    >
                      {a.population}
                    </div>
                    <div
                      className="text-[10px] text-white/30 mt-1 truncate"
                      style={{ fontFamily: "var(--font-mono)" }}
                    >
                      {a.last_action}
                    </div>
          ...
          {selected && (
            <div className="px-5 py-4 animate-fade-in">
              <div className="flex items-center justify-between mb-3">
                <div
                  className="text-[10px] tracking-widest font-bold text-white/30"
                  style={{ fontFamily: "var(--font-mono)" }}
                >
          ...
          <div className="px-5 py-4">
            <div
              className="text-[10px] tracking-widest font-bold text-white/30 mb-3"
              style={{ fontFamily: "var(--font-mono)" }}
            >
              ENV
            </div>
            <div className="grid grid-cols-3 gap-2">
              <EnvReadout
                label="TEMP"
                value={tick.environment.temperature.toFixed(2)}
                unit="°C"
                color="#00cfff"
              />
              <EnvReadout
                label="NUTRIENTS"
                value={tick.environment.nutrients.toFixed(2)}
                color="#00ff9d"
              />
              <EnvReadout label="pH" value={tick.environment.pH.toFixed(2)} color="#00cfff" />
              <EnvReadout
                label="SALINITY"
                value={tick.environment.salinity.toFixed(1)}
                unit="PSU"
                color="#00cfff"
              />
              <EnvReadout
                label="FISHING"
                value={tick.environment.fishing_pressure.toFixed(2)}
                color={tick.environment.fishing_pressure > 0.4 ? "#ff4757" : "#00ff9d"}
              />
              <EnvReadout
                label="POLLUTION"
                value={tick.environment.pollution_index.toFixed(2)}
                color={tick.environment.pollution_index > 0.4 ? "#ff4757" : "#ffb347"}
              />
            </div>
          </div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-3 text-[9px]" style={{ fontFamily: "var(--font-mono)" }}>
                <span className="flex items-center gap-1">
                  ...
                  <span className="text-white/40">FISHING</span>
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-2 h-2 rounded-full" style={{ background: "#ffb347" }} />
                  <span className="text-white/40">POLLUTION</span>
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-2 h-2 rounded-full" style={{ background: "#00ff9d" }} />
                  <span className="text-white/40">NUTRIENTS</span>
                </span>
              </div>
            </div>
          ...
          <div className="px-5 py-4">
            ...
            <div
              className="text-center text-[10px] text-white/25 tracking-widest"
              style={{ fontFamily: "var(--font-mono)" }}
            >
...
const API_URL = "http://35.238.167.25:8000";
const SIM_TICKS = 10;
